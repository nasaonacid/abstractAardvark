...
from abstractAardvark.serializers import NodeSerializer,NodeSerializerDummy, TreeSerializer, TreeSerializerDummy, PaginatedTreeSerializer, PaginatedTreeSerializerDummy
...
def game_list(request,format = None):
    """
    List all trees or create new tree
    ---

    request_serializer: TreeSerializerDummy
    response_serializer: TreeSerializerDummy

    responseMessages:
    - code: 400
      message: Posted Tree is invalid
...
def game_control(request,pk = None,diff = 'easy', format = None):
    """
    Retrieve a new game or verify answers
    Please note that the Nested Serializer is not used but is autogenerated. The node serializer children is just an array with the same fields as the node serializer
...
def difficulty_list(request, diff, username = None,  format = None):
    """
    Get trees made by difficulty with further possiblity to reduce number by user
    ---
