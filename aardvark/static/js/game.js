/*
Created by James Stewart 1000039s

Where is everything????

 hierrarchyLayer == empty tree structure
 answerLayer == answer boxes
 continueLayer == title box, difficulty complete box, game finish
 filterLayer == blacked out background for completion   
 optionLayer == scores and difficulty select
...
function drawDiffComplete(){
    ...
    var width = messageBox.children[0].width();
    var height = messageBox.children[0].height();
    var firstButton = drawButton(x+(width*0.7),y+(height*0.7),choices[0],items[0].inner, items[0].outer)
    var secondButton = drawButton(x+(width*0.1),y+(height*0.7),choices[1],items[1].inner, items[1].outer)
    ...
    continueLayer.children[0].setAttr('opacity',0.75)
    // continueLayer.draw()    // filterLayer.draw()
}
...
function drawFinish(){
    ...
    var width = messageBox.children[0].width();
    var height = messageBox.children[0].height();
    var continueButton = drawButton(x+(width*0.7),y+(height*0.7),"submit score","#5ABE66", "#1E8C2C")
    var quitButton = drawButton(x+(width*0.1),y+(height*0.7),"quit","#D41C1C","#910000")
...
function drawTitle(){
    ...
    var width = messageBox.children[0].width();
    var height = messageBox.children[0].height();
    var easyButton = drawButton(x+(width*0.1),y+(height*0.7),"easy","#9B88DF", "#4429A1")
    var mediumButton = drawButton(x+(width*0.4),y+(height*0.7),"medium","#FFEE8F","#E9CC25")
    var hardButton = drawButton(x+(width*0.7),y+(height*0.7),"hard","#D41C1C","#910000")
...
function drawOptions(){
    ...
    // diffText = drawText((stageWidth*0.5)-radius*3,y-radius, "choose your difficulty:")
...
function drawContinue(){
    ...
    var continueButton = drawButton((x + (width*0.7)),(y+(height*0.75)),">","#5ABE66", "#1E8C2C", true)
    var quitButton = drawButton((x + (width*0.1)),(y+(height*0.75)),"X","#D41C1C","#910000", true)
    ...
    filterLayer.setAttr('opacity',0.5)
    filterLayer.children[0].setAttr('visible', true)
    continueLayer.children[0].setAttr('opacity', 0.85)
    continueLayer.draw()
    filterLayer.draw()
...
*/
function drawButton(x,y, text, inner, outer, cort){
    ...
    if (cort){
        content.setAttr("fontSize",rect.height());
        content.setAttr("fill","white");
    }
    else{
